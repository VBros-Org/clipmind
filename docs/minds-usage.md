...
- 2026-07-30: Builders-chat intel: MENTE_SendToMind (Mind-to-Mind token transfer) reportedly worked in the past but multiple builders now hit a bounds error (runtime quotes rate-converted limits ~1.1551x the published schema bounds). Relevance to ClipMind: if/when fixed, a funded Mind could seed newborn creator Minds with starter cognition at onboarding (the treasury model for our invite-gating economics). Status: WATCH ONLY - do not build against it until we can execute a transfer ourselves. Not posted publicly (uncertain diagnosis, George's call).
- 2026-07-30: T27 channel pull live-verified ON PROD: yt-dlp listed and fetched from Railway (not IP-blocked today), 3 videos transcribed, Mind created + seeded on a throwaway creator (probe Mind da33443e disabled after). yt_blocked degradation retained as the guard.
